...
exports.fetchRedditBike = async (richEmbed, msg) => {
  const url = `https://www.reddit.com/r/FixedGearBicycle.json?sort=controversial&t=week&type=link`
  try {
    const data = await fetch(url),
      json = await data.json()
    ...
    msg.channel.send("Something went wrong, try again!")
    console.error("Reddit Fixie Error: ", e)
  }
}
